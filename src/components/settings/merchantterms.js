...
const MerchantTerms = ({setting, getSettings, setMerchant}) => {
  ...
  const [params, setParams] = useState({
    terms:"",
    token,
    action:"update_merchant_terms"
  }) 
  useEffect(() => {
   setParams((prev) => ({...prev, terms:setting?.merchant_terms}))
  }, [setting])
  ...
  const updateTerms = async (e) => {
    ...
    if(res?.status === 'successful') {
      ...
      isPending('save_mer', "Save and Publish");
    }else {
      // setToast('Terms Updated Successfully');
      isPending('save_mer', "Save and Publish");
    }
  ...
  return ( <>
  <>
   <div className="add-item-modal" onClick={()=> {setMerchant(false)}}>
    <div className="my-col-6 off-3 my-bottom-50 bg-white down-5 rad-10" onClick={(e) => {e.stopPropagation()}}>
      <div className="my-col-10 off-1 down-3 px13 bold bd-bottom"><span>Merchant Terms</span></div>
        ...
        <form>
          <div className="mother">
           ...
           />
            ...
            <button className="my-btn-sm bd-unset down-4 bg-color-code-1 px13 white" id='save_mer' onClick={(e) => {updateTerms(e)}} >Save and Publish</button>
          </div>
        </form>
      </div>
    </div>
