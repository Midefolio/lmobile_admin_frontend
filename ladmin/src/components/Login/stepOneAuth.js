   ...
   <div className="my-col-10 off-1 down-8"> 
    ...
    <div className="mother down-4">
     ...
     <div className="faded my-col-6">
      ...
      <div className="mother down-1"><input type="password" ref={passwordRef} id="password" className="input" value={loginParams?.password}  onChange={(e)=> {setLoginParams(prev => ({...prev, password:e.target.value}))}}/></div>
     </div>
     <div className="mother down-5"><span className="submit-btn bg-color-code-1" id='login_btn' onClick={Login}>Login</span></div>
    </div>
    <div className="my-col-12 down-5 centered"><span className="pd-5 c-pointer color-code-1 my-btn-sm px13">forgotten Password ?</span></div>
   </div>
  </div>
 </>  );
}
