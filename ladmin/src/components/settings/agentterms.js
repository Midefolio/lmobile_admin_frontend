...
const AgentTerms = ({setting, getSettings, setAgent}) => {
  ...
  const [params, setParams] = useState({
    terms:"",
    token,
    action:"update_agent_terms"
  }) 
  useEffect(() => {
   setParams((prev) => ({...prev, terms:setting?.agent_terms}))
  }, [setting])
  ...
  const updateTerms = async (e) => {
    ...
    if(res?.status === 'successful') {
      ...
      isPending('save_agent', "Save and Publish");
    }else {
      // setToast('Terms Updated Successfully');
      isPending('save_agent', "Save and Publish");
    }
  ...
  return ( <>
  <>
   <div className="add-item-modal" onClick={()=> {setAgent(false)}}>
    <div className="my-col-6 off-3 my-bottom-50 bg-white down-5 rad-10" onClick={(e) => {e.stopPropagation()}}>
      <div className="my-col-10 off-1 down-3 px13 bold bd-bottom"><span>Agent Terms</span></div>
        ...
        <form action="">
          <div className="mother">
           ...
           />
            ...
            <button className="my-btn-sm bd-unset down-4 bg-color-code-1 px13 white" id='save_agent' onClick={(e) => {updateTerms(e)}} >Save and Publish</button>
          </div>
        </form>
      </div>
    </div>
